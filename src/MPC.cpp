...
size_t psi_start = y_start + N; // ψ
size_t v_start = psi_start + N; // ν
size_t cte_start = v_start + N; // e
size_t epsi_start = cte_start + N; // ψe
size_t delta_start = epsi_start + N; // δ
size_t a_start = delta_start + N - 1; // a
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = state.size() * N + 2 * (N - 1); // 2 actuators
